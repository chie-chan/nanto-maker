import { useState, useRef } from "react";
import { useSearchParams, useNavigate } from "react-router-dom";
import ShareBar from "./ShareBar";

// ── あいまい一致 ────────────────────────────────────────────
function toHiragana(str: string): string {
  return str.replace(/[\u30A1-\u30F6]/g, c =>
    String.fromCharCode(c.charCodeAt(0) - 0x60)
  );
}
function normalize(s: string) {
  return toHiragana(s.trim().replace(/\s+/g, "").toLowerCase());
}
function isMatch(answer: string, input: string): boolean {
  if (!input.trim()) return false;
  const a = normalize(answer);
  const i = normalize(input);
  return a === i || a.includes(i) || i.includes(a);
}

// ── 結果メッセージ ──────────────────────────────────────────
function getMessage(correct: number, total: number): string {
  if (correct === total)              return "ふふ。あなた、わかってるわね。🌹";
  if (correct >= Math.ceil(total * 0.6)) return "まあ。もう少しで及第点よ。💅";
  if (correct >= 1)                  return "あら。まだまだ修行が必要ね。😤";
  return "まあ！出直してらっしゃい。👋";
}

// ── クイズデータ encode / decode ────────────────────────────
interface QuizData {
  name: string;
  questions: Array<{ q: string; a: string }>;
}
function encodeQuiz(data: QuizData): string {
  return btoa(encodeURIComponent(JSON.stringify(data)));
}
function decodeQuiz(s: string): QuizData | null {
  try { return JSON.parse(decodeURIComponent(atob(s))); } catch { return null; }
}

// ── 結果画像（集中線） ──────────────────────────────────────
function drawQuizResult(
  canvas: HTMLCanvasElement,
  creatorName: string,
  correct: number,
  total: number,
  message: string
) {
  const S = 1080;
  canvas.width = S;
  canvas.height = S;
  const ctx = canvas.getContext("2d")!;

  // 白背景
  ctx.fillStyle = "#fff";
  ctx.fillRect(0, 0, S, S);

  // 集中線
  const cx = S / 2, cy = S / 2;
  const maxR = S * 0.87;
  const count = 120;
  for (let i = 0; i < count; i++) {
    const angle = (i / count) * Math.PI * 2;
    const spread = (Math.PI / count) * 0.55;
    const r0 = 28 + Math.random() * 22;
    ctx.beginPath();
    ctx.moveTo(cx + Math.cos(angle - spread) * r0, cy + Math.sin(angle - spread) * r0);
    ctx.lineTo(cx + Math.cos(angle) * maxR, cy + Math.sin(angle) * maxR);
    ctx.moveTo(cx + Math.cos(angle + spread) * r0, cy + Math.sin(angle + spread) * r0);
    ctx.lineTo(cx + Math.cos(angle) * maxR, cy + Math.sin(angle) * maxR);
    ctx.strokeStyle = `rgba(0,0,0,${0.45 + Math.random() * 0.55})`;
    ctx.lineWidth = 1 + Math.random() * 2.5;
    ctx.stroke();
  }

  // 中央黄色ボックス
  const bw = 880, bh = 500;
  const bx = (S - bw) / 2, by = (S - bh) / 2;
  ctx.fillStyle = "#FFE600";
  ctx.fillRect(bx, by, bw, bh);
  ctx.strokeStyle = "#111";
  ctx.lineWidth = 9;
  ctx.strokeRect(bx, by, bw, bh);

  ctx.textAlign = "center";
  ctx.fillStyle = "#111";

  // 出題者名
  ctx.font = "bold 44px 'Arial Black', Arial, sans-serif";
  ctx.fillText(`${creatorName}のクイズ`, cx, by + 88);

  // 仕切り線
  ctx.fillRect(bx + 50, by + 108, bw - 100, 5);

  // スコア
  ctx.font = "900 108px 'Arial Black', Arial, sans-serif";
  ctx.fillText(`${correct}/${total}問正解！`, cx, by + 268);

  // 仕切り線2
  ctx.fillStyle = "rgba(0,0,0,0.12)";
  ctx.fillRect(bx + 50, by + 298, bw - 100, 3);
  ctx.fillStyle = "#111";

  // メッセージ（句点で2行に分ける）
  ctx.font = "bold 38px 'Arial Black', Arial, sans-serif";
  const parts = message.split("。").filter(Boolean);
  if (parts.length >= 2) {
    ctx.fillText(parts[0] + "。", cx, by + 375);
    ctx.fillText(parts.slice(1).join("。"), cx, by + 430);
  } else {
    ctx.fillText(message, cx, by + 405);
  }

  // アプリ名
  ctx.font = "600 26px Arial, sans-serif";
  ctx.fillStyle = "#999";
  ctx.fillText("🐾 うちのこメーカー", cx, S - 44);
}

// ── Props ───────────────────────────────────────────────────
interface Props { isMobile: boolean; dark: boolean; text: string; bg: string; }

// ── メインコンポーネント ────────────────────────────────────
export default function QuizPage({ isMobile, dark, text, bg }: Props) {
  const [params] = useSearchParams();
  const quizParam = params.get("q");
  const quizData = quizParam ? decodeQuiz(quizParam) : null;

  if (quizData) {
    return <QuizPlayer isMobile={isMobile} dark={dark} text={text} bg={bg} data={quizData} />;
  }
  return <QuizCreator isMobile={isMobile} dark={dark} text={text} bg={bg} />;
}

// ── 出題者画面 ──────────────────────────────────────────────
function QuizCreator({ isMobile, dark, text, bg }: Props) {
  const navigate = useNavigate();
  const [name, setName] = useState("");
  const [questions, setQuestions] = useState([
    { q: "", a: "" },
    { q: "", a: "" },
    { q: "", a: "" },
  ]);
  const [generatedUrl, setGeneratedUrl] = useState("");
  const [copied, setCopied] = useState(false);

  const addQuestion = () => setQuestions(qs => [...qs, { q: "", a: "" }]);
  const removeQuestion = (i: number) => setQuestions(qs => qs.filter((_, idx) => idx !== i));
  const updateQ = (i: number, field: "q" | "a", val: string) =>
    setQuestions(qs => qs.map((item, idx) => idx === i ? { ...item, [field]: val } : item));

  const generateUrl = () => {
    if (!name.trim()) { alert("名前を入力してください"); return; }
    const filled = questions.filter(q => q.q.trim() && q.a.trim());
    if (filled.length === 0) { alert("問題と答えを1つ以上入力してください"); return; }
    const data: QuizData = { name: name.trim(), questions: filled };
    const url = `${window.location.origin}/quiz?q=${encodeQuiz(data)}`;
    setGeneratedUrl(url);
  };

  const copyUrl = async () => {
    try {
      await navigator.clipboard.writeText(generatedUrl);
    } catch {
      const el = document.createElement("textarea");
      el.value = generatedUrl;
      document.body.appendChild(el);
      el.select();
      document.execCommand("copy");
      document.body.removeChild(el);
    }
    setCopied(true);
    setTimeout(() => setCopied(false), 2500);
  };

  const inp = (extra?: object) => ({
    border: "3px solid #111",
    padding: "10px 12px",
    fontSize: 15,
    fontFamily: "'Arial Black', sans-serif",
    background: "#fff",
    color: "#111",
    width: "100%",
    boxSizing: "border-box" as const,
    outline: "none",
    ...extra,
  });

  return (
    <div>
      {/* ナビ */}
      <div style={{ display: "flex", borderBottom: "3px solid #111", background: "#FFE600", alignItems: "center" }}>
        <div style={{
          padding: isMobile ? "8px 12px" : "10px 22px",
          fontSize: isMobile ? 12 : 14, fontWeight: 900, color: "#111",
        }}>🌹 あなた、わかってる？</div>
        <button onClick={() => navigate("/")} style={{
          marginLeft: "auto", padding: isMobile ? "8px 12px" : "10px 18px",
          fontSize: isMobile ? 11 : 13, fontWeight: 700,
          border: "none", background: "transparent", cursor: "pointer", color: "#555",
        }}>← ホームへ</button>
      </div>

      <div style={{ maxWidth: 600, margin: "0 auto", padding: isMobile ? "24px 16px" : "36px 24px" }}>

        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: isMobile ? 20 : 24, fontWeight: 900, letterSpacing: -0.5, marginBottom: 6 }}>
            あなた、わかってる？メーカー
          </div>
          <div style={{ fontSize: 12, color: "#888", lineHeight: 1.6 }}>
            クイズを作ってURLをシェア。友達がどれだけ知ってるか試そう🌹
          </div>
        </div>

        {/* 名前 */}
        <SectionLabel>あなたの名前</SectionLabel>
        <input
          placeholder="例：あいこ"
          value={name}
          onChange={e => setName(e.target.value)}
          style={{ ...inp(), marginBottom: 24 }}
        />

        {/* 問題リスト */}
        <SectionLabel>問題と答え</SectionLabel>
        {questions.map((item, i) => (
          <div key={i} style={{
            border: "3px solid #111", padding: 14, marginBottom: 12,
            background: "#fff", position: "relative",
          }}>
            <div style={{ fontSize: 10, fontWeight: 900, letterSpacing: 2, color: "#999", marginBottom: 8 }}>
              Q{i + 1}
            </div>
            <input
              placeholder="質問（例：好きな食べ物は？）"
              value={item.q}
              onChange={e => updateQ(i, "q", e.target.value)}
              style={{ ...inp(), marginBottom: 8 }}
            />
            <input
              placeholder="答え（例：バナナ）"
              value={item.a}
              onChange={e => updateQ(i, "a", e.target.value)}
              style={{ ...inp({ background: "#FFF9CC" }) }}
            />
            {questions.length > 1 && (
              <button onClick={() => removeQuestion(i)} style={{
                position: "absolute", top: 8, right: 10,
                border: "none", background: "none",
                fontSize: 16, cursor: "pointer", color: "#bbb",
              }}>✕</button>
            )}
          </div>
        ))}

        <button onClick={addQuestion} style={{
          width: "100%", padding: "13px",
          border: "3px dashed #bbb", background: "transparent",
          fontSize: 14, fontWeight: 900, cursor: "pointer",
          color: "#888", marginBottom: 20,
          fontFamily: "'Arial Black', sans-serif",
        }}>＋ 問題を追加</button>

        <button onClick={generateUrl} style={{
          width: "100%", padding: "16px",
          border: "3px solid #111", background: "#FFE600",
          fontSize: 16, fontWeight: 900, cursor: "pointer",
          color: "#111", marginBottom: 20,
          fontFamily: "'Arial Black', sans-serif",
          letterSpacing: 0.5,
        }}>クイズURLを生成する →</button>

        {generatedUrl && (
          <div style={{ border: "4px solid #111", padding: 16, background: "#fff" }}>
            <div style={{ fontSize: 10, fontWeight: 900, letterSpacing: 3, color: "#999", marginBottom: 8 }}>
              生成されたURL
            </div>
            <div style={{
              fontSize: 12, wordBreak: "break-all", color: "#333",
              background: "#f5f5f5", padding: 10, marginBottom: 12, lineHeight: 1.6,
            }}>{generatedUrl}</div>
            <button onClick={copyUrl} style={{
              width: "100%", padding: "14px",
              border: "3px solid #111",
              background: copied ? "#4CAF50" : "#111",
              color: copied ? "#fff" : "#FFE600",
              fontSize: 15, fontWeight: 900, cursor: "pointer",
              fontFamily: "'Arial Black', sans-serif",
              transition: "background 0.2s",
            }}>{copied ? "✓ コピーしました！" : "URLをコピー"}</button>
            <div style={{ fontSize: 11, color: "#888", marginTop: 10, textAlign: "center" }}>
              このURLをSNSやDMでシェアしよう🐾
            </div>
          </div>
        )}
      </div>
    </div>
  );
}

// ── 挑戦者画面 ──────────────────────────────────────────────
function QuizPlayer({ isMobile, dark, text, bg, data }: Props & { data: QuizData }) {
  const navigate = useNavigate();
  const [answers, setAnswers] = useState<string[]>(data.questions.map(() => ""));
  const [phase, setPhase] = useState<"play" | "result">("play");
  const [results, setResults] = useState<boolean[]>([]);
  const canvasRef = useRef<HTMLCanvasElement>(null);

  const submit = () => {
    const r = data.questions.map((q, i) => isMatch(q.a, answers[i]));
    setResults(r);
    setPhase("result");
    setTimeout(() => {
      if (!canvasRef.current) return;
      const correct = r.filter(Boolean).length;
      drawQuizResult(canvasRef.current, data.name, correct, data.questions.length, getMessage(correct, data.questions.length));
    }, 50);
  };

  const retry = () => {
    setPhase("play");
    setAnswers(data.questions.map(() => ""));
    setResults([]);
  };

  const getBlob = (): Promise<Blob | null> =>
    new Promise(resolve => {
      if (!canvasRef.current) { resolve(null); return; }
      canvasRef.current.toBlob(resolve, "image/png");
    });

  const correct = results.filter(Boolean).length;
  const message = phase === "result" ? getMessage(correct, data.questions.length) : "";

  return (
    <div style={{ maxWidth: 600, margin: "0 auto", padding: isMobile ? "24px 16px" : "36px 24px" }}>

      {phase === "play" && (
        <>
          <div style={{ marginBottom: 28 }}>
            <div style={{
              display: "inline-block",
              background: "#e07050", color: "#fff",
              fontSize: 12, fontWeight: 900, padding: "4px 10px",
              letterSpacing: 1, marginBottom: 8,
            }}>
              {data.name}からの挑戦状 🌹
            </div>
            <div style={{ fontSize: isMobile ? 22 : 26, fontWeight: 900, letterSpacing: -0.5, marginBottom: 4 }}>
              あなた、わかってる？
            </div>
            <div style={{ fontSize: 12, color: "#888" }}>
              全{data.questions.length}問・答えを入力してね
            </div>
          </div>

          {data.questions.map((q, i) => (
            <div key={i} style={{ marginBottom: 18 }}>
              <div style={{
                background: "#111", color: "#FFE600",
                padding: "10px 14px", fontWeight: 900, fontSize: 15, marginBottom: 8,
              }}>
                Q{i + 1}. {q.q}
              </div>
              <input
                placeholder="答えを入力..."
                value={answers[i]}
                onChange={e => setAnswers(a => a.map((v, idx) => idx === i ? e.target.value : v))}
                onKeyDown={e => { if (e.key === "Enter" && i === data.questions.length - 1) submit(); }}
                style={{
                  border: "3px solid #111", padding: "12px 14px",
                  fontSize: 16, width: "100%", boxSizing: "border-box",
                  fontFamily: "'Arial Black', sans-serif",
                  background: "#fff", color: "#111", outline: "none",
                }}
              />
            </div>
          ))}

          <button onClick={submit} style={{
            width: "100%", padding: "18px",
            border: "3px solid #111", background: "#FFE600",
            fontSize: 20, fontWeight: 900, cursor: "pointer",
            color: "#111", marginTop: 8,
            fontFamily: "'Arial Black', sans-serif",
            letterSpacing: 1,
          }}>どん！</button>
        </>
      )}

      {phase === "result" && (
        <>
          {/* 結果サマリー */}
          <div style={{
            background: "#111", color: "#FFE600",
            padding: "20px", marginBottom: 20, textAlign: "center",
          }}>
            <div style={{ fontSize: 13, marginBottom: 4 }}>{data.name}のクイズ 結果発表</div>
            <div style={{ fontSize: isMobile ? 32 : 40, fontWeight: 900 }}>
              {correct}/{data.questions.length}問正解！
            </div>
            <div style={{ fontSize: 15, marginTop: 8, color: "#fff" }}>{message}</div>
          </div>

          {/* 各問の正誤 */}
          <div style={{ marginBottom: 20 }}>
            {data.questions.map((q, i) => (
              <div key={i} style={{
                border: `3px solid ${results[i] ? "#4CAF50" : "#e07050"}`,
                padding: "12px 14px", marginBottom: 10,
                background: results[i] ? "#f0fff0" : "#fff0f0",
              }}>
                <div style={{ fontSize: 12, color: "#888", marginBottom: 4, fontWeight: 900 }}>
                  Q{i + 1}. {q.q}
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ fontSize: 14 }}>
                    <span style={{ color: "#888" }}>あなた：</span>
                    <span style={{ fontWeight: 900 }}>{answers[i] || "（未回答）"}</span>
                  </div>
                  <span style={{ fontSize: 22 }}>{results[i] ? "✅" : "❌"}</span>
                </div>
                {!results[i] && (
                  <div style={{ fontSize: 12, color: "#e07050", marginTop: 4, fontWeight: 700 }}>
                    正解：{q.a}
                  </div>
                )}
              </div>
            ))}
          </div>

          {/* 結果画像 */}
          <canvas ref={canvasRef} style={{ width: "100%", border: "4px solid #111", display: "block", marginBottom: 10 }} />
          <ShareBar getBlob={getBlob} dark={dark} />

          {/* アクションボタン */}
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            <button onClick={retry} style={{
              flex: 1, padding: "12px",
              border: "3px solid #111", background: "transparent",
              fontSize: 13, fontWeight: 900, cursor: "pointer",
              color: "#111", fontFamily: "'Arial Black', sans-serif",
            }}>もう一度挑戦</button>
            <button onClick={() => navigate("/quiz")} style={{
              flex: 1, padding: "12px",
              border: "3px solid #111", background: "#111",
              fontSize: 13, fontWeight: 900, cursor: "pointer",
              color: "#FFE600", fontFamily: "'Arial Black', sans-serif",
            }}>自分もクイズを作る</button>
          </div>
        </>
      )}
    </div>
  );
}

function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ fontSize: 10, fontWeight: 900, letterSpacing: 3, color: "#999", textTransform: "uppercase", marginBottom: 8 }}>
      {children}
    </div>
  );
}
